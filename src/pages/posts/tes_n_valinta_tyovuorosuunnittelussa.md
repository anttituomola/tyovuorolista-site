---
layout: ../../layouts/MarkdownPostLayout.astro
title: 'TES:n valinta työvuorosuunnittelussa, mitä "Ei mikään näistä" tarkoittaa?'
pubDate: 2026-07-20
description: 'Työehtosopimuksen valinta ohjaa tyovuorolista.fi:n TES-varoituksia ja palkkaraporttien tuntikategorioita. Tämä opas kertoo, mistä valinta tehdään, mitä "Ei mikään näistä" tarkoittaa ja mitä valinnan vaihtaminen vaikuttaa.'
author: 'Antti Tuomola'
image:
    url: '/blogPostImages/tes-valinta/rekisteroityminen-tes-valinta.png'
    alt: 'Rekisteröitymislomake, jossa työehtosopimuksen valintavalikko on auki: MaRa, Kaupan ala, Kiinteistöpalveluala, SOSTES ja vaihtoehto "Ei mikään näistä / en osaa sanoa".'
tags: ['TES', 'työehtosopimus', 'opas', 'uudet ominaisuudet']
---

**Lyhyesti:** [TES-tuessa](/posts/tes_tuki_tyovuorosuunnittelussa) kaikki
lähtee yhdestä valinnasta: mitä työehtosopimusta toimipaikkasi noudattaa.
Valinta kysytään jo rekisteröityessä, ja se määrää, miten sovellus
varoittaa vuorosuunnittelun virheistä ja luokittelee tunnit
palkanlaskentaan. Jos et ole varma, valitse **"Ei mikään näistä / en osaa
sanoa"**. Mikään ei mene rikki, ja valinnan voi tehdä tai vaihtaa
myöhemmin asetuksista.

## Miksi TES:n valinnalla on väliä?

Työehtosopimus määrää työvuorosuunnittelun pelisäännöt: työ- ja
lepoajat, vuorojen pituudet, vapaapäivät, lisä- ja ylityörajat sekä
erilaiset lisät. Ne vaihtelevat aloittain. Ravintola-alan
[kolmiviikkoisjakso ja 112,5 tunnin
raja](/posts/kolmiviikkoisjakso_ravintola-alalla) on eri asia kuin kaupan
alan viikkotyöaikamalli.

Kun olet valinnut toimipaikallesi TES:n, sovellus:

- **varoittaa suunnitteluvaiheessa**, jos vuoro rikkoo työaikalakia tai
  TES:n määräyksiä. Punainen on lakiasia, oranssi TES-asia ja sininen
  huomio, eikä mikään niistä koskaan estä tallentamista
- **luokittelee toteutuneet tunnit** automaattisesti palkanlaskennan
  kategorioihin: lisätyöt, ylityöt 50 % ja 100 %, V- ja X-päivätyöt sekä
  lisät eriteltyinä
- **vie kategoriat [palkkaraporttiin](/posts/palkkaraportti_ominaisuus)**
  sellaisenaan, palkkalajikoodein varustettuna

Ilman valintaa sovellus ei oleta alaasi, eikä siten voi tietää, mitä
sääntöjä listojasi pitäisi verrata vasten.

## Valinta tehdään jo rekisteröityessä

Uutta tiliä luotaessa lomake kysyy yrityksen nimen lisäksi noudatettavaa
työehtosopimusta:

<img src="/blogPostImages/tes-valinta/rekisteroityminen-tes-valinta.png" alt="Rekisteröitymislomake: yrityksen nimi täytettynä ja TES-valintavalikko auki, vaihtoehtoina neljä työehtosopimusta ja 'Ei mikään näistä / en osaa sanoa'" width="100%" style="max-width: 900px; display: block; margin: 20px auto;" />

Valikossa on neljä sopimusta: **Matkailu-, ravintola- ja vapaa-ajan
palveluiden TES (MaRa)**, **Kaupan alan TES**, **Kiinteistöpalvelualan
TES** ja **Yksityisen sosiaalipalvelualan TES (SOSTES)**. Oletuksena on
"Ei mikään näistä / en osaa sanoa", joten valinta ei koskaan jää
rekisteröitymisen esteeksi, se tallentuu kuitenkin joka tapauksessa,
jotta alakohtaiset ominaisuudet voidaan tarjota myöhemmin oikealle
yleisölle.

## Mistä tiedät, mitä työehtosopimusta noudatatte?

Jos alasi TES ei ole heti selvä, helpoin tapa tarkistaa on katsoa
**työsopimusta tai työnantajan antamaa kirjallista selvitystä työsuhteen
ehdoista**. Työsopimuslaki (2 luvun 4 §) velvoittaa työnantajan
ilmoittamaan siinä muun muassa työhön sovellettavan työehtosopimuksen.

Kaksi asiaa kannattaa tietää taustalla:

- **Työnantajaliittoon kuulumattomankin on noudatettava alansa
  yleissitovaa työehtosopimusta** vähimmäisehtoina (työsopimuslaki 2
  luvun 7 §). Eli vaikka yrityksesi ei olisi järjestäytynyt, alan TES:n
  minimiehdot koskevat todennäköisesti silti sinua.
- Valtioneuvosto vahvistaa yleissitovat työehtosopimukset, ja ne on
  listattu [Finlexin työehtosopimussivulla](https://www.finlex.fi/fi/viranomaiset/tyoehtosopimukset).
  Sieltä näet, onko alallasi yleissitova TES.

Jos asia jää epäselväksi, kysy palkanlaskijaltasi tai alan
työntekijäliitosta (esimerkiksi PAM palvelualoilla). Väärällä
valinnalla saadut varoitukset ovat hyödyttömämpiä kuin ei
varoituksia ollenkaan.

## Mitä "Ei mikään näistä / en osaa sanoa" tarkoittaa?

Se on tarkoituksella turvallinen oletus, ei virhe. Kun valinta on "Ei
mikään näistä":

- kalenteri **ei näytä TES-varoituksia** lainkaan
- tunteja **ei luokitella** lisä- ja ylityö- tai lisäkategorioihin
- kaikki muu toimii täsmälleen kuten ennen TES-tukea: listat,
  julkaisut, tuntien kirjaus ja raportit toimivat normaalisti

Valitse tämä myös silloin, jos alasi sopimus puuttuu vielä valikosta, ja kerro siitä minulle, niin tiedän, mitä alaa kannattaa työstää
seuraavaksi. Valinnan voi vaihtaa koska tahansa myöhemmin.

## Näin vaihdat valintaa myöhemmin

TES-valintaa voi muuttaa kohdasta **Hallinta → Asetukset →
Työehtosopimus**. Muutos tallentuu heti, kun valitset vaihtoehdon:

<img src="/blogPostImages/tes-valinta/asetukset-tes-valinta.png" alt="Asetukset-sivun Työehtosopimus-osio: valittuna Matkailu-, ravintola- ja vapaa-ajan palveluiden TES, alla linkki tarkempiin asetuksiin" width="100%" style="max-width: 700px; display: block; margin: 20px auto;" />

Kun TES on valittuna, samasta kohdasta aukeaa linkki **Hallinta →
Työehtosopimus** -sivulle, jossa tehdään tarkemmat asetukset:
[jaksorytmin alkumaanantai](/posts/nain_otat_tes-tuen_kayttoon) (MaRa),
[tasoittumisjärjestelmä](/posts/tasoittumisjarjestelma_ravintola-alalla),
paikalliset sopimukset perusteluineen ja palkkalajikoodien mappaus.
Samalta sivulta löytyy myös **Poista TES käytöstä** -painike, joka
palauttaa toimipaikan "ei valintaa" -tilaan.

## Mitä valinnan vaihtaminen vaikuttaa?

Vaihto on kevyt operaatio, mutta hyvä ymmärtää sen vaikutukset:

- **Varoitukset seuraavat uutta TES:ää heti.** Kalenteri tulkitsee
  listat jatkossa uuden sopimuksen säännöillä, esimerkiksi ravintolan
  ja kaupan vuoron pituus- ja leposäännöt eroavat toisistaan.
- **Tuntien luokittelu muuttuu.** Lisä- ja ylityörajat sekä lisät
  lasketaan uuden TES:n mukaan, ja palkkaraportin kategoriat seuraavat
  samaa muutosta.
- **Vuorot, työntekijät ja tuntikirjaukset eivät muutu.** Vaihto ei
  koske tallennettuihin listoihin eikä työntekijöiden tietoihin, vain
  sääntöihin, joilla sovellus tulkitsee niitä. Työntekijöiden
  [sopimustunnit](/posts/tyotunnit_tyosopimuksessa) ja muut TES-tiedot
  säilyvät, joten palaaminen aiempaan valintaan ei vaadi tiedon
  syöttämistä uudelleen.

Eri sopimusten tuki on eri kypsyysasteella: **MaRa** on pilotissa
pisimmällä (kolmiviikkoisjaksojen täysi lisä- ja ylityölaskenta,
vuosivapaaseuranta, tasoittuminen), **Kaupan alan TES** kattaa lisät,
vuoro- ja leposäännöt sekä
[alan omat työaikamallit](/posts/kaupan_alan_tes_tyovuorosuunnittelussa),
ja **Kiinteistöpalvelualan** sekä **SOSTES:in** tuki laajenee pilotin
edetessä. Jos huomaat, että alasi sopimuksessa puuttuu jotain, palaute
tulee suoraan minulle ja vaikuttaa työstöjärjestykseen.

## Mitä TES-valinta ei tee

TES-tuki on suunnittelun apuväline, ei vaatimustenmukaisuustakuu.
Sovellus tarkistaa listasi valitun sopimuksen säännöillä, mutta
työehtosopimuksen tulkinta on lopulta työnantajan vastuulla, ja
paikalliset sopimukset voivat muuttaa määräyksiä tavoilla, joita
sovellus tuntee vain, jos ne on kirjattu asetuksiin. Varoitusten
tarkoitus on, ettei mikään jää huomaamatta; päätökset jäävät aina
sinulle.

TES-apuri (beta) sisältyy kaikkiin paketteihin. Valitse työehtosopimus
asetuksista, niin varoitukset ja palkkaraportin TES-luokittelu tulevat
käyttöön. Katso [näin otat TES-tuen käyttöön](/posts/nain_otat_tes-tuen_kayttoon).
Jos alasi sopimus puuttuu valikosta, laita viestiä
[info@tyovuorolista.fi](mailto:info@tyovuorolista.fi).

---

*Lähteet: [Työsopimuslaki 55/2001 (Finlex)](https://www.finlex.fi/fi/laki/ajantasa/2001/20010055), [Finlex: Yleissitovat työehtosopimukset](https://www.finlex.fi/fi/viranomaiset/tyoehtosopimukset), [MaRa-TES (tes.mara.fi)](https://tes.mara.fi/), [PAM: Työehtosopimukset](https://www.pam.fi/tyoelama/tyoehtosopimukset.html)*
